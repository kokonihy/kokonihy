##########################################################################################
# - writer : kokonihy@naver.com
# - https://github.com/kokonihy/namuh_stock_dividend_parser
##########################################################################################
...
class GUI:
    def us_stock_balance_input(self):
        str_title = '해외주식잔고 엑셀을 선택해주세요'
        us_stock_balance = filedialog.askopenfilename(initialdir='./', title=str_title, filetypes=(('*.xlsx','*xlsx'),('*.xls','*xls')))
        if us_stock_balance == '':
            messagebox.showwarning('경고', '해외주식잔고 엑셀파일이 선택되지 않았습니다. 재시도해주세요.')
            sys.exit()
        print('--해외주식잔고 엑셀파일 : ', us_stock_balance)

        return us_stock_balance

    def deal_detail_input(self):
        str_title = '거래내역 엑셀을 선택해주세요'
        deal_detail = filedialog.askopenfilename(initialdir='./', title=str_title, filetypes=(('*.xlsx','*xlsx'),('*.xls','*xls')))
        if deal_detail == '':
            messagebox.showwarning('경고', '거래내역 엑셀파일이 선택되지 않았습니다. 재시도해주세요.')
            sys.exit()
        print('--거래내역 엑셀파일 : ', deal_detail)

        return deal_detail


def stock_balance_sorting(us_file):
    df_us = pd.read_excel(us_file, engine='openpyxl')

    # 수정포인트 : 농협엑셀양식의 dataframe.keys 값이 이상한게 많으므로, 이걸로 확인후 아래 for문 수정 필요
    print('--농협 해외주식잔고 엑셀의 DataFrame key값 리스트--\n', df_us.keys(), end='\n\n')

    len_df_us_row = len(df_us['종목명'])
...
def stock_balance_add_dividend_info(df_stock_balance, df_dividend_detail):
    ...
    def sum_monthly_dividend(df, len_df):
        for row in range(0, len_df):
            ...
            list_monthly_dividend = [x for x in list_monthly_dividend if numpy.isnan(x) == False]
    ...
    df_other_dividend_detail = pd.DataFrame(data=None, index=range(0, int(len(df_dividend_detail['종목명']))), columns=['입금일자', '종목명', '외화입금액', '환율', '원화입금액'])
...
def bar_graph_create(df, year, flag):
    if flag == '월별배당금':
        ...
        fig_name = './' + str(year) + '년_월별_배당금.png'
        ...
        fig_name = './' + str(year) + '년_종목별_배당금.png'
        plt.savefig(fig_name)
        plt.cla()
...
def main():
    gui = GUI()
    us_stock_balance = gui.us_stock_balance_input()
    deal_detail = gui.deal_detail_input()

    df_stock_balance = stock_balance_sorting(us_stock_balance)
    df_dividend_detail = deal_detail_sorting(deal_detail)
    ...
    df_dividend_detail.to_excel('./일자별 배당금입금내역.xlsx')
    # df_other_dividend_detail.to_excel('./잔고외 배당금입금내역.xlsx')

    df_stock_balance_writer = pd.ExcelWriter('./잔고내 배당금입금내역.xlsx')
